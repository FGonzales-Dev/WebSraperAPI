...
def balance_sheet_json(request):
    if 'ticker' in request.GET and 'market' in request.GET:
        os.remove("testy.csv")
        html_content = get_stock_balance_sheet(request, request)
...
def stock_history_key_ratio_json(request):
    if 'ticker' in request.GET and 'market' in request.GET:
        ...
        header_name = ["name", "year_one", "year_two","year_three","year_four","year_five","year_six","year_seven","year_eight","year_nine","year_ten","year_ttm","yearsss"]
